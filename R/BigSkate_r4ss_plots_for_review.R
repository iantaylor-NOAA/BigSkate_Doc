...
sens.sum_STAR1 <- SSsummarize(list(bs82,
                                  bs99,
                                  bs101))
sens.names_STAR1 <- c("old base",
                     "new prior (with 1% and 99% quantiles)",
                     "new prior (with 1% and 99% quantiles) + Dirichlet-Multinomial")
# make comparison plot
SSplotComparisons(sens.sum_STAR1,
                  plot = FALSE,
                  print = TRUE,
                  subplot = 2,
                  filenameprefix = "sens.STAR1_",
                  yaxs = 'i', ylimAdj = 1.1, 
                  endyrvec = 2019, legendlabels = sens.names_STAR1,
                  legendloc = 'topright',
                  indexfleets = 5,
